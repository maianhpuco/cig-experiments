...
class Generic_MIL_Dataset_SlideName(Dataset):
    def __init__(self,
                 data_dir,
                 csv_path=None,
                 label_dict=None,
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_h5:
            full_path = os.path.join(data_dir, 'pt_files', f"{slide_id}.pt")
            features = torch.load(full_path, weights_only=True)
            return features, label, None, slide_id
        else:
            full_path = os.path.join(data_dir, 'h5_files', f"{slide_id}.h5")
            with h5py.File(full_path, 'r') as hdf5_file:
                features = hdf5_file['features'][:]
...
def return_splits_custom_with_slidename(csv_path, data_dir, label_dict, seed=1, print_info=False, use_h5=False):
    """Create train, val, and test datasets from a custom split CSV."""
    df = pd.read_csv(csv_path)
    ...
    def create_dataset(col_case, col_label):
        case_ids = df[col_case].dropna().tolist()
        labels = df[col_label].dropna().astype(int).tolist()
        return Generic_MIL_Dataset_SlideName(
            data_dir=data_dir,
            case_ids=case_ids,
            labels=labels,
            label_dict=label_dict,
