...
class CIG(CoreSaliency):
    ...
    def GetMask(self, **kwargs):
        ...
        x_value = x_value.to(device, dtype=torch.float32) if isinstance(x_value, torch.Tensor) else torch.tensor(x_value, dtype=torch.float32, device=device)
        baseline_features = baseline_features.to(device, dtype=torch.float32) if isinstance(baseline_features, torch.Tensor) else torch.tensor(baseline_features, dtype=torch.float32, device=device)
        ...
        for alpha in tqdm(alphas, desc=f"Computing class {target_class_idx}:", ncols=100):
            ...
            # Forward pass without detaching graph
