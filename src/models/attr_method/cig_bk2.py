...
def call_model_function(inputs, model, call_model_args=None):
    ...
    if call_model_args and call_model_args.get("attention_only", False):
        A = model(inputs, attention_only=True)  # [1, N]
        print(f"call_model_function: attention shape={A.shape}, norm={torch.norm(A):.4f}, max={A.max():.4f}, min={A.min():.4f}")
        return A

    logits = model(inputs)  # [1, C]
    print(f"call_model_function: inputs shape={inputs.shape}, requires_grad={inputs.requires_grad}, "
          f"logits shape={logits.shape}, requires_grad={logits.requires_grad}")
    return logits 
...
class CIG(CoreSaliency):
    ...
    def GetMask(self, **kwargs):
        ...
        for step_idx, alpha in enumerate(tqdm(alphas, desc="Computing:", ncols=100), start=1):
            # Scale input to prevent Tanh saturation
            x_step_batch = (baseline_features + alpha * x_diff).clone().detach()
            x_step_batch = (x_step_batch - x_step_batch.mean()) / (x_step_batch.std() + 1e-8)  # Standardize
            ...
            # Modified loss: L2 norm + class-specific term
            l2_loss = torch.norm(logits_step - logits_r, p=2) ** 2
            class_loss = -torch.log_softmax(logits_step, dim=1)[0, target_class_idx]
            loss = l2_loss + 0.5 * class_loss  # Increased weight for class loss
